...
export default function WhatsAppIcon() {
  ...
  return (
    <>
      {isOpen && (
        <div className="wa-chat-window animate-slide-up">
          <div className="wa-chat-header">
            <div>
              <h4 className="wa-chat-title">LULU AURELIAN </h4>
              <p className="wa-chat-subtitle">We reply in minutes</p>
            </div>
            <button className="wa-close-btn" onClick={() => setIsOpen(false)} aria-label="Close chat">×</button>
          </div>
          <div className="wa-chat-body">
            <p className="wa-greeting">Hello! How can we help you today?
      ...
      <button 
        className={`floating-whatsapp ${isOpen ? 'active' : ''}`}
        onClick={() => setIsOpen(!isOpen)}
        aria-label="Contact Concierge on WhatsApp"
      >
        {!isOpen && <div className="pulse-ring" />}
        {!isOpen && <span className="wa-tooltip">Chat with Concierge</span>}
        <svg 
